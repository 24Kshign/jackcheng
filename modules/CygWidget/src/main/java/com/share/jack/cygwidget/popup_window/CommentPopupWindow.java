...
public class CommentPopupWindow extends BasePopupWindow implements View.OnClickListener {
    ...
    public void showPopupWindow(View v) {
        try {
            ...
            mPopupWindow.showAtLocation(v, Gravity.RIGHT | Gravity.TOP, (int) (v.getWidth() * 1.8),
                    viewLocation[1] - CygDisplayMetrics.dp2px(v.getContext(), 15f));

            if (getAnimation() != null && getAnimationView() != null) {
                getAnimationView().startAnimation(getAnimation());
            }
        } catch (Exception e) {
            Log.w("error", "error");
        }
    ...
    public View getPopupView() {
        return LayoutInflater.from(mContext).inflate(R.layout.pw_comment_popupwindow, null);
    }
